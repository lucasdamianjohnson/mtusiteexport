...
function download_files($xml,$extension,$debug_file,$ou_root,$parent) {
	...
	if (strripos($url->loc, $extension) != FALSE) {
		...
		if ($cursor_old != $cursor) {
			$debug_converted = "redirect 301 magazine/$cursor_old$extension htttp://www.mtu.edu/magazine/$cursor$extension<br>"; 
...
foreach ($xml->url as $url) {
	...
	//$page = file_get_html($url->loc);
		...
		if ($page != '') {
			...
			//download the content from the magazine 
			$body_content_for_mag = $page->find('article[id=content_body]',0);
			//remove the title from the page
			if ($body_content_for_mag == "") {
		
			} else {
			
			$body_content_for_mag->find('h1',0)->outertext = "";
			$body_content_for_mag = str_replace("</>", "</p>", $body_content_for_mag);

			//echo $body_content_for_mag;
		}
			//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//\/\/\/\/\/\/\/\/\/
			...
			$nav_for_mag = $page->find('div[id=site-nav] ul ul',0);
			...
			echo "<br><strong><h1>did not find page</h1></strong>";
...
<parameter name=\"page-external-include\" type=\"text\" rows=\"3\" group=\"Everyone\" prompt=\"External Include\" alt=\"Included in the head of this page.\" section=\"External Includes\"><link type=\"text/css\" rel=\"stylesheet\" href=\"//www.mtu.edu/mtu_resources/styles/n/pubs.css\"/></parameter>
...
<ouc:editor csspath=\"/_resources/ou/editor/wysiwyg.css\" cssmenu=\"/_resources/ou/editor/styles.txt\" wysiwyg-class=\"maincontent\"/>$body_content_for_mag
</ouc:div>
...
$nav = "<!-- ouc:editor csspath=\"/_resources/ou/editor/nav.css\" cssmenu=\"/_resources/ou/editor/styles-nav.txt\" wysiwyg-class=\"navigation\" --><li><a href=\""."/".$ou_root."/".$cursor."\">Current Stories</a></li>".$nav_for_mag;
	...
	<document xmlns:ouc=\"http://omniupdate.com/XSL/Variables\">
		...
		<option value=\"true\" selected=\"true\">Restart Navigation</option></parameter>
...
echo "<h1> putting pcf in => files/".$ou_root."/".$cursor."index.pcf</h1>";
